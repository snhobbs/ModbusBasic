...
TEST(CalculateBytesRemaining, ReadHoldingRegisters) {
  ...
  Modbus::Frame frame{0x0f, Modbus::Function::kWriteMultipleHoldingRegisters, data.size(),
                            ArrayView<uint8_t>{data.size(), data.data()}};
  for (uint8_t i = 1; i > 0; i++) {
    frame.data_array[Modbus::WriteMultipleHoldingRegistersCommand::CommandPacket::kNumberOfDataBytes] = i;
